...
type UserQuery struct {
	...
	// eager-loading edges.
	withDepts            *DeptQuery
	withPropertiesInDept *UserPropertyInDeptQuery
...
func (uq *UserQuery) QueryDepts() *DeptQuery {
...
// QueryPropertiesInDept chains the current query on the "properties_in_dept" edge.
func (uq *UserQuery) QueryPropertiesInDept() *UserPropertyInDeptQuery {
	query := &UserPropertyInDeptQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withDepts:            uq.withDepts.Clone(),
		withPropertiesInDept: uq.withPropertiesInDept.Clone(),
...
func (uq *UserQuery) WithDepts(opts ...func(*DeptQuery)) *UserQuery {
	query := &DeptQuery{config: uq.config}
...
// WithPropertiesInDept tells the query-builder to eager-load the nodes that are connected to
// the "properties_in_dept" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithPropertiesInDept(opts ...func(*UserPropertyInDeptQuery)) *UserQuery {
	query := &UserPropertyInDeptQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withPropertiesInDept = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withDepts; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[string]*User, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.Depts = []*Dept{}
		}
		var (
			edgeids []uint
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(user.DeptsPrimaryKey[1], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, uq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "depts": %w`, err)
		}
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "depts" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Depts = append(nodes[i].Edges.Depts, n)
			}
		}
	}

	if query := uq.withPropertiesInDept; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[string]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.PropertiesInDept = []*UserPropertyInDept{}
		}
		query.Where(predicate.UserPropertyInDept(func(s *sql.Selector) {
			s.Where(sql.InValues(user.PropertiesInDeptColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			...
			node.Edges.PropertiesInDept = append(node.Edges.PropertiesInDept, n)
		}
	}
