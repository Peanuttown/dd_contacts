...
// QueryUserPropertiesInDept chains the current query on the "user_properties_in_dept" edge.
func (dq *DeptQuery) QueryUserPropertiesInDept() *UserPropertyInDeptQuery {
	query := &UserPropertyInDeptQuery{config: dq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := dq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (dq *DeptQuery) FirstID(ctx context.Context) (id uint, err error) {
	var ids []uint
...
func (dq *DeptQuery) FirstIDX(ctx context.Context) uint {
...
func (dq *DeptQuery) OnlyID(ctx context.Context) (id uint, err error) {
	var ids []uint
...
func (dq *DeptQuery) OnlyIDX(ctx context.Context) uint {
...
func (dq *DeptQuery) IDs(ctx context.Context) ([]uint, error) {
	var ids []uint
...
func (dq *DeptQuery) IDsX(ctx context.Context) []uint {
	ids, err := dq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
// WithUserPropertiesInDept tells the query-builder to eager-load the nodes that are connected to
// the "user_properties_in_dept" edge. The optional arguments are used to configure the query builder of the edge.
func (dq *DeptQuery) WithUserPropertiesInDept(opts ...func(*UserPropertyInDeptQuery)) *DeptQuery {
	query := &UserPropertyInDeptQuery{config: dq.config}
	for _, opt := range opts {
		opt(query)
	}
...
//		Scan(ctx, &v)
//
func (dq *DeptQuery) Select(field string, fields ...string) *DeptSelect {
...
func (dq *DeptQuery) sqlAll(ctx context.Context) ([]*Dept, error) {
	...
	if query := dq.withUsers; query != nil {
		...
		ids := make(map[uint]*Dept, len(nodes))
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(dept.UsersPrimaryKey[0], fks...))
			},
	...
	if query := dq.withUserPropertiesInDept; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[uint]*Dept)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.UserPropertiesInDept = []*UserPropertyInDept{}
		}
		query.Where(predicate.UserPropertyInDept(func(s *sql.Selector) {
			s.Where(sql.InValues(dept.UserPropertiesInDeptColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
...
func (dq *DeptQuery) sqlQuery(ctx context.Context) *sql.Selector {
	...
	selector := builder.Select(t1.Columns(dept.Columns...)...).From(t1)
	if dq.sql != nil {
		selector = dq.sql
		selector.Select(selector.Columns(dept.Columns...)...)
	}
...
func (ds *DeptSelect) sqlQuery() sql.Querier {
	selector := ds.sql
